...
// 0.1.0: pre-run validator for flow-tab graphs - 12 rules flag unreachable
...
dependencies {
    ...
    // Test dependencies for rule unit assertions.
...
tasks.register<Jar>("buildPluginJar") {
    archiveFileName.set("boss-plugin-flow-lint-${version}.jar")
    duplicatesStrategy = DuplicatesStrategy.EXCLUDE

    manifest {
        attributes(
            "Implementation-Title" to "BOSS Plugin Flow Lint Plugin",
            "Implementation-Version" to version,
            "Main-Class" to "ai.rever.boss.plugin.dynamic.flowlint.FlowLintDynamicPlugin"
        )
    }
